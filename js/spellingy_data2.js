...
zvyraznit:zvyraznit, dalsiMoznosti:dalsiMoznosti
	...
	angular
	.module("spellingy")
	.factory("laemeFactory",function($log,$http){
	
	
	function lexelForms(lx,fce)
	{
		
		$http({method:"POST",
		      url:"laemeData.php",
		       data:{fce:fce,lx:lx,gr:"",txts:""}})
			.then(function(response){
				
				$log.info("LexelForms: "+response.data);
			})
